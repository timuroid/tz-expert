...
- errors[]: объект для **каждого** E-кода в группе (даже если нарушений нет).
  ...
  - instances[]:
    ...
    - sections: полное локальное название ровно одного раздела, как оно написано в документе. Если существует более глубокий подраздел, используйте его, а не родительский. Если ошибка отностится к разделу которого не существует, напиши название раздела, который нужно создать.
    - fix: как справить ошибку, что и куда внести (конкретно).
...
- В `fix` формулируйте конкретное действие для единственного адресата (из `instances.sections`), без «или».
